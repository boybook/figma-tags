<template>
  <span class="tag" :style="{ padding: removable ? '0 0 0 8px' : '0 8px', color: 'rgba(' + colorText.r + ',' + colorText.g + ',' + colorText.b + ',' + colorText.a + ')', background: 'rgba(' + colorBackground.r + ',' + colorBackground.g + ',' + colorBackground.b + ',' + colorBackground.a + ')' }">
    {{ tag.name }}
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const { colorBackground, colorText } = useTagColors(props.tag, isDark);
    return { clickRemove, colorBackground, colorText }
  }
}
</script>
